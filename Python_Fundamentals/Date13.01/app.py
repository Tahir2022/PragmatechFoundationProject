...
def login ():
    ...
    if 3 < len(name) < 11:
        ...
        if 5 < len(surname) < 15:
            year = input('Pls add your bith year: ')
            if len(year) == 4:
                email = input('Pls add your mail addres: ')
                if 10 < len(email) < 22:
                    password = input('Pls add password: ')
                    if 6 < len(password) < 13:
                        submit_pass = input('Submit password again: ')
                        if password == submit_pass:
                            return
